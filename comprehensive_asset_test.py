...
This version creates required dependencies (properties) first.

Authentication: admin@habitere.com / admin123
Backend URL: https://plan-builder-8.preview.emergentagent.com/api
"""
...
class ComprehensiveAssetTester:
    def __init__(self):
    ...
    def create_test_property(self):
        ...
        print("🏠 CREATING TEST PROPERTY FOR ASSET DEPENDENCIES...")
        
        property_data = {
            "title": "Test Property for Asset Management",
            "description": "A test property created specifically for asset management testing. This property will be used to link assets and test the complete asset management workflow.",
            "price": 50000000.0,
            "location": "Douala, Cameroon",
            "property_sector": "Commercial Properties",
            "property_category": "Office Buildings for Sale",
            "listing_type": "sale",
            "bedrooms": 0,
            "bathrooms": 2,
            "area_sqm": 500.0,
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.created_properties.append(created_property['id'])
                self.log_test("Create Test Property", True, 
                            f"Created property: {created_property['title']}", response.status_code)
                return created_property['id']
            else:
                self.log_test("Create Test Property", False, 
                            f"Failed: {response.text}", response.status_code)
                return None
        except Exception as e:
            self.log_test("Create Test Property", False, f"Exception: {str(e)}")
            return None

    def test_assets_endpoints(self, property_id):
        """Test all 6 Assets endpoints"""
        print("🏢 TESTING ASSETS ENDPOINTS (6 total)...")
        
        if not property_id:
            print("⚠️ No property available for asset testing")
            return
    ...
    def test_cascade_delete(self):
        ...
        if self.created_properties:
            ...
            try:
                response = self.session.delete(f"{BASE_URL}/properties/{property_id}")
                if response.status_code == 200:
                    self.log_test("DELETE /properties/{property_id} - Clean Up Test Property", True, 
                                "Test property deleted successfully", response.status_code)
                else:
                    self.log_test("DELETE /properties/{property_id} - Clean Up Test Property", False, 
                                f"Failed: {response.text}", response.status_code)
            except Exception as e:
                self.log_test("DELETE /properties/{property_id} - Clean Up Test Property", False, f"Exception: {str(e)}")
    ...
    def run_all_tests(self):
        ...
        # Create test property for asset dependencies
        property_id = self.create_test_property()
        
        # Run all endpoint tests
        self.test_assets_endpoints(property_id)
...
if __name__ == "__main__":
    tester = ComprehensiveAssetTester()
