...
import { Home, MapPin, DollarSign, BedDouble, Bath, Square, Upload, X, Image as ImageIcon, ChevronDown, Phone, MessageSquare } from 'lucide-react';
...
const PropertyForm = () => {
  ...
  const handleFileSelect = async (e) => {
    ...
    try {
      setError('Processing images... (Auto-cropping to 16:9)');
      
      // Create previews immediately (original files)
      ...
      setSelectedFiles(prev => [...prev, ...tempPreviews]);
      
      // Process images immediately (crop + compress)
      const processedFiles = await Promise.all(
        files.map(async (file, idx) => {
          try {
            console.log('Cropping and compressing:', file.name);
            const processed = await compressImage(file);
            console.log('Processed:', processed.name, 'Size:', processed.size);
            ...
            return tempPreviews[idx];
          }
        })
      );
      
      // Replace temp previews with processed images
      setSelectedFiles(prev => {
        const filtered = prev.filter(f => !tempPreviews.find(t => t.name === f.name));
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="flex flex-col-reverse sm:flex-row gap-3 sm:gap-4 pt-6">
              <button
                ...
                className="flex-1 px-6 py-3 sm:py-3 border-2 border-gray-300 text-gray-700 font-semibold rounded-xl hover:bg-gray-50 active:bg-gray-100 transition-colors"
              >
                Cancel
              </button>
              <button
                type="submit"
                disabled={loading}
                className="flex-1 px-6 py-3 sm:py-3 bg-gradient-to-r from-blue-600 to-purple-600 text-white font-semibold rounded-xl hover:from-blue-700 hover:to-purple-700 active:from-blue-800 active:to-purple-800 disabled:opacity-50 disabled:cursor-not-allowed transition-all shadow-lg hover:shadow-xl"
              >
